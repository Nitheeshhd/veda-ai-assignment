...
export default function Home() {
  ...
  const [output, setOutput] = useState<Section[]>([]);
  ...
  const generateQuestions = (): Section[] => {
    return [
      {
        section: "Section A",
        questions: [
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        "https://veda-ai-assignment-old3.onrender.com/user",
        {
          method: "POST",
          headers: {
      ...
      // Generate questions after successful save
      setOutput(generateQuestions());
